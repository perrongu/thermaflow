...
function getRELaminarMax() {
  return _flowRegimes_nusselt
    ? _flowRegimes_nusselt.RE_LAMINAR_MAX
    : window.FlowRegimes.RE_LAMINAR_MAX;
}

function getRETurbulentMin() {
  return _flowRegimes_nusselt
    ? _flowRegimes_nusselt.RE_TURBULENT_MIN
...
function nusseltHausen(Re, Pr, D, L) {
  ...
  if (Re >= 2300) {
    console.warn(
      `Hausen: Re=${Re} ≥ 2300 (hors régime laminaire), utiliser corrélation turbulente`
    );
  }
  if (Pr < 0.6) {
  ...
  // Corrélation de Hausen
  const Nu =
    HAUSEN_CONSTANT_BASE +
    (HAUSEN_CONSTANT_0668 * param) /
      (1 + HAUSEN_CONSTANT_004 * Math.pow(param, HAUSEN_EXP_CORRECTION));
...
function nusseltGnielinski(Re, Pr, f = null) {
  ...
  const denominator =
    ...
    GNIELINSKI_CONSTANT_12_7 *
      Math.sqrt(f_used / GNIELINSKI_CONSTANT_8) *
      (Math.pow(Pr, GNIELINSKI_EXP_PR) - 1);
