...
(function() {
  ...
  const DEFAULT_RANGE_PERCENT = 0.20; // ±20%
    ...
    if (paramKey === 'm_dot') {
      ...
      if (typeof window.WaterProperties !== 'undefined') {
        try {
          ...
          console.warn(`Impossible d'obtenir rho_water à T=${T_water}°C, P=${P_water} bar. Utilisation: ${rho_water} kg/m³`);
        }
      }
  ...
  function validateRanges() {
    ...
    if (isNaN(xMin)) {
      ...
      state.validationErrors.push(`${paramDefX.label} (X) min: doit être entre ${paramDefX.min} et ${paramDefX.max} ${paramDefX.unit}`);
      elements.rangeXMin.classList.add('error');
    } else {
    ...
    if (isNaN(xMax)) {
      ...
      state.validationErrors.push(`${paramDefX.label} (X) max: doit être entre ${paramDefX.min} et ${paramDefX.max} ${paramDefX.unit}`);
      elements.rangeXMax.classList.add('error');
    } else {
    ...
    if (isNaN(yMin)) {
      ...
      state.validationErrors.push(`${paramDefY.label} (Y) min: doit être entre ${paramDefY.min} et ${paramDefY.max} ${paramDefY.unit}`);
      elements.rangeYMin.classList.add('error');
    } else {
    ...
    if (isNaN(yMax)) {
      ...
      state.validationErrors.push(`${paramDefY.label} (Y) max: doit être entre ${paramDefY.min} et ${paramDefY.max} ${paramDefY.unit}`);
      elements.rangeYMax.classList.add('error');
    } else {
    ...
    if (state.validationErrors.length > 0) {
      elements.errors.innerHTML = state.validationErrors.map(err => 
  ...
  function calculateMatrix(rangeX, rangeY, resolution) {
    ...
    for (let j = 0; j < resolution; j++) {
      ...
      for (let i = 0; i < resolution; i++) {
        ...
        const adjustedConfig = adjustConfigForStability(config, valuesX[i], valuesY[j], paramDefX, paramDefY);
          ...
          try {
            const fallbackConfig = createFallbackConfig(config, valuesX[i], valuesY[j], paramDefX, paramDefY);
    ...
    if (errors.length > 0) {
      console.warn(`⚠️ ${errorCount} calculs hors plage physique (affichés en gris). Erreurs:`, errors.slice(0, 3));
    }
    console.log(`📊 Matrice ${resolution}x${resolution} calculée: ${successCount} valides, ${errorCount} invalides`);
  ...
  function drawImprovedLegend(ctx, width, height, padding) {
    ...
    const legendItems = [
      { color: '#FFD6D6', label: window.I18n ? I18n.t('chart.legendFreeze') : 'Gel (≤ 0°C)' },
      { color: '#FFF4CC', label: window.I18n ? I18n.t('chart.legendUnder') : 'Sous marge (0-5°C)' },
      { color: '#DFFFD6', label: window.I18n ? I18n.t('chart.legendSafe') : 'Sécuritaire (≥ 5°C)' },
      { color: '#cccccc', label: window.I18n ? I18n.t('chart.legendInvalid') : 'Invalide (hors plage physique)' }
    ];
