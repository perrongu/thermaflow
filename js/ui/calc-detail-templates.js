/**
 * calc-detail-templates.js
 *
 * HTML template generation functions for the calculation details section.
 *
 * Extracted from calculation-details.js to keep each file under 500 LOC.
 * This module is responsible for pure HTML string generation only — no DOM
 * manipulation, no event handling, no LaTeX rendering.
 *
 * Depends on: window.I18n, window.UnitConverter, window.MaterialProperties,
 *             window.UIUtils (accessed via window.* for browser/Node compat)
 */
...
(function () {
   ...
   */
  function toScientificLatex(value, decimals) {
   ...
   * @param {Object} _networkResult - Résultats du réseau (non utilisé, réservé)
   * @param {Object} config - Configuration de l'analyse
   * @returns {string} HTML string
   */
  function generateExecutiveSummary(_networkResult, config) {
    const segmentLength = config.totalLength / config.numSegments;

    const t = (key, replacements) => (window.I18n ? window.I18n.t(key, replacements) : key);
   ...
   * @param {number} segmentNum - Numéro du segment (1-based)
   * @param {number} startPosition - Position de début en mètres
   * @param {Object} segmentResult - Résultat du segment
   * @param {Object} config - Configuration
   * @param {number} totalSegments - Nombre total de segments
   * @returns {string} HTML string
   */
  function generateSegmentHeader(segmentNum, startPosition, segmentResult, config, totalSegments) {
    const segmentLength = config.totalLength / config.numSegments;
   ...
   * @param {number} T_avg - Température moyenne du fluide en °C
   * @param {number} P_bar - Pression en bar
   * @param {Object} water - Propriétés de l'eau
   * @param {Object} air - Propriétés de l'air
   * @param {number} T_amb - Température ambiante en °C
   * @returns {string} HTML string
   */
  function displayFluidProperties(T_avg, P_bar, water, air, T_amb) {
    const t = (key) => (window.I18n ? window.I18n.t(key) : key);

    // Convertir pression: bar → kPag → unité d'affichage
    const P_kPag = P_bar * 100; // 1 bar = 100 kPa
    const P_display = window.UnitConverter
      ? window.UnitConverter.fromSI('pressure', P_kPag)
      : P_kPag;
    const pressureUnit = window.UnitConverter
      ? window.UnitConverter.getUnitInfo('pressure').label
      : 'kPag';
   ...
   * @param {Object} result - Résultat du segment (calculé)
   * @param {Object} geometry - Géométrie du segment
   * @param {Object} config - Configuration
   * @param {Object} water - Propriétés de l'eau
   * @returns {string} HTML string
   */
  function displayHydraulics(result, geometry, config, water) {
    const t = (key) => (window.I18n ? window.I18n.t(key) : key);
            ...
            <div class="calc-block__result">
              ${(() => {
                ...
                const dP_display = window.UnitConverter
                  ? window.UnitConverter.fromSI('pressure', dP_kPa)
                  : dP_kPa;
                const pUnit = window.UnitConverter
                  ? window.UnitConverter.getUnitInfo('pressure').label.replace('g', '')
                  : 'kPa';
                return `${t('calcDetails.step2.pressureDrop.result')} ${result.dP.toFixed(1)} Pa (${dP_display.toFixed(2)} ${pUnit})`;
              })()}
            </div>
            <div class="calc-block__reference">
   ...
   * @param {Object} result - Résultat du segment
   * @param {Object} geometry - Géométrie du segment
   * @param {Object} config - Configuration
   * @param {Object} water - Propriétés de l'eau
   * @returns {string} HTML string
   */
  function displayConvectionInternal(result, geometry, config, water) {
    const t = (key) => (window.I18n ? window.I18n.t(key) : key);
    ...
    // Suppress unused variable warning — config is reserved for future use
    void config;
   ...
   * @param {Object} result - Résultat du segment
   * @param {Object} config - Configuration
   * @param {Object} _air - Propriétés de l'air (réservé)
   * @returns {string} HTML string
   */
  function displayConvectionExternal(result, config, _air) {
    const t = (key) => (window.I18n ? window.I18n.t(key) : key);
    ...
    const pipeMat = window.MaterialProperties.getMaterialProperties(config.geometry.material);
   ...
   * @returns {string} HTML string
   */
  function displayThermalResistances(result, geometry, config) {
    const t = (key) => (window.I18n ? window.I18n.t(key) : key);
    const pipeMat = window.MaterialProperties.getMaterialProperties(geometry.material);
    ...
    if (config.insulation) {
      const insulMat = window.MaterialProperties.getMaterialProperties(config.insulation.material);
      const D_o_insul = D_o + 2 * config.insulation.thickness;
      const materialI18nKey = window.UIUtils
        ? window.UIUtils.getInsulationI18nKey(config.insulation.material)
   ...
   * @param {Object} result - Résultat du segment (calculé complet)
   * @param {Object} segmentResult - Résultat du segment (du réseau)
   * @param {Object} config - Configuration
   * @param {Object} water - Propriétés de l'eau
   * @returns {string} HTML string
   */
  function displayNTU(result, segmentResult, config, water) {
    const t = (key) => (window.I18n ? window.I18n.t(key) : key);
  ...
  const CalcDetailTemplates = {
    toScientificLatex: toScientificLatex,
    generateExecutiveSummary: generateExecutiveSummary,
    generateSegmentHeader: generateSegmentHeader,
    displayFluidProperties: displayFluidProperties,
    displayHydraulics: displayHydraulics,
    displayConvectionInternal: displayConvectionInternal,
    displayConvectionExternal: displayConvectionExternal,
    displayThermalResistances: displayThermalResistances,
    displayNTU: displayNTU,
  };

  // Export global (browser)
  window.CalcDetailTemplates = CalcDetailTemplates;

  // Export pour Node.js (tests)
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = CalcDetailTemplates;
  }
})();
