...
(function() {
  ...
  const PARAMETER_DEFINITIONS = {
    'L': { 
      get label() { return getParameterLabel('L'); },
      unit: 'm',
      path: ['totalLength'],
      min: 1,
      max: 1000
    ...
    'm_dot': { 
      get label() { return getParameterLabel('m_dot'); },
      get unit() {
      ...
      convertToSI: (value) => window.UnitConverter ? UnitConverter.toSI('flowRate', value) : value,
      convertFromSI: (value) => window.UnitConverter ? UnitConverter.fromSI('flowRate', value) : value
    },
    'T_in': { 
      get label() { return getParameterLabel('T_in'); },
      unit: '°C',
      path: ['fluid', 'T_in'],
      min: 1,
      max: 100
    ...
    'T_amb': { 
      get label() { return getParameterLabel('T_amb'); },
      unit: '°C',
      path: ['ambient', 'T_amb'],
      min: -40,
      max: 50
    ...
    'V_wind': { 
      get label() { return getParameterLabel('V_wind'); },
      unit: 'km/h',
      path: ['ambient', 'V_wind'],
      min: 0,
      max: 108
  ...
  function analyzeSensitivity1D(baseConfig) {
    ...
    for (const [paramKey, paramDef] of Object.entries(PARAMETER_DEFINITIONS)) {
      ...
      for (let attempt = 0; attempt < 5; attempt++) {
        try {
          ...
          if (effectiveMin !== minToUse) {
            console.log(`✓ MIN ajusté pour ${paramKey}: ${minToUse.toFixed(2)} → ${effectiveMin.toFixed(2)}`);
          ...
          if (attempt === 4) {
            ...
            effectiveMin = effectiveMin * 1.10;
          }
        }
      }
      ...
      for (let attempt = 0; attempt < 5; attempt++) {
        try {
          ...
          if (effectiveMax !== maxToUse) {
            console.log(`✓ MAX ajusté pour ${paramKey}: ${maxToUse.toFixed(2)} → ${effectiveMax.toFixed(2)}`);
          ...
          if (attempt === 4) {
            ...
            effectiveMax = effectiveMax * 0.90;
          }
        }
      }
      ...
      const criticalValues = findCriticalParameterValues(baseConfig, paramKey, effectiveParamDef, T_base, baseValue, T_atMin, T_atMax);
   ...
   */
  function findCriticalParameterValues(baseConfig, paramKey, paramDef, T_base, baseValue, T_atMin, T_atMax) {
    ...
    if (T_base <= FREEZE_TEMP) {
      ...
      if (increasingParamIncreasesTemp) {
        ...
        result.freeze = findParameterValueInRange(baseConfig, paramKey, paramDef, FREEZE_TEMP, baseValue, paramDef.max);
      } else {
        // Diminuer le paramètre améliore → chercher entre min et base
        result.freeze = findParameterValueInRange(baseConfig, paramKey, paramDef, FREEZE_TEMP, paramDef.min, baseValue);
      }
    } else {
    ...
    if (T_base <= SAFETY_THRESHOLD) {
      ...
      if (increasingParamIncreasesTemp) {
        ...
        result.safety = findParameterValueInRange(baseConfig, paramKey, paramDef, SAFETY_THRESHOLD, baseValue, paramDef.max);
      } else {
        // Diminuer améliore → chercher entre min et base
        result.safety = findParameterValueInRange(baseConfig, paramKey, paramDef, SAFETY_THRESHOLD, paramDef.min, baseValue);
      }
    } else {
   ...
   */
  function findParameterValueInRange(baseConfig, paramKey, paramDef, targetTemp, searchMin, searchMax) {
    let min = searchMin;
    let max = searchMax;
    ...
    for (let i = 0; i < maxIterations; i++) {
      ...
      try {
        ...
        if ((T_mid < targetTemp && T_min < targetTemp) || (T_mid > targetTemp && T_min > targetTemp)) {
          min = mid;
          T_min = T_mid;
        } else {
          max = mid;
  ...
  function drawTornadoChart(canvasId, result, baseConfig) {
    ...
    if (needsBadge) {
      ...
      if (!badge) {
        ...
        badge.style.cssText = 'font-size: 10px; color: #6b7280; margin-top: 4px; text-align: center; font-style: italic;';
      ...
      if (truncation.shouldTruncate) {
        ...
        const truncLabel = window.I18n ? I18n.t('sensitivity.truncatedRange') : 'Plage tronquée pour lisibilité';
        const truncDetail = truncation.reason || (window.I18n ? I18n.t('sensitivity.truncatedDetail') : 'Centrée sur valeurs importantes');
        ...
        const effectiveLabel = window.I18n ? I18n.t('sensitivity.effectiveRange') : 'Plage effective';
        const theoreticalLabel = window.I18n ? I18n.t('sensitivity.theoreticalRange') : 'Plage théorique';
        const exceedsLabel = window.I18n ? I18n.t('sensitivity.exceedsLimits') : 'dépasse limites physiques';
   ...
   */
  function drawColoredBar(ctx, startX, barY, barWidth, barHeight, result, displayMin, displayMax, baseConfig) {
    ...
    if (T_atDisplayMin === null || T_atDisplayMax === null) {
      ...
      const detailText = T_atMin === null && T_atMax === null ? 
        ' (min et max)' : 
        T_atMin === null ? ' (min)' : ' (max)';
    ...
    if (safetyParamValue !== null && safetyParamValue > displayMin && safetyParamValue < displayMax) {
      transitions.push({ param: safetyParamValue, T: SAFETY_THRESHOLD });
    }
    
    if (freezeParamValue !== null && freezeParamValue > displayMin && freezeParamValue < displayMax) {
  ...
  function drawReferenceLines(ctx, barY, barHeight, result, xScale) {
    ...
    labels.push({ x: baseX, text: 'Base', color: '#000000', font: 'bold 9px sans-serif', priority: 1 });
    ...
    if (result.criticalValueFreeze !== null) {
      ...
      labels.push({ x: freezeX, text: '0°C', color: '#dc2626', font: '8px sans-serif', priority: 2 });
    }
    ...
    if (result.criticalValueSafety !== null) {
      ...
      labels.push({ x: safetyX, text: '5°C', color: '#f59e0b', font: '8px sans-serif', priority: 3 });
    }
   ...
   */
  function drawValueLabels(ctx, barY, barHeight, result, xScale, paddingBottom) {
  ...
  function generateSummaryTable(results) {
    ...
    results.forEach(result => {
      ...
      const T_minFormatted = result.T_atMin !== null ? (result.T_atMin >= 0 ? '+' : '') + result.T_atMin.toFixed(1) + '°C' : 'N/A';
      const T_maxFormatted = result.T_atMax !== null ? (result.T_atMax >= 0 ? '+' : '') + result.T_atMax.toFixed(1) + '°C' : 'N/A';
      
      const outOfRangeText = window.I18n ? I18n.t('detailedCalcs.outOfRange') : 'Hors plage';
      
      const freezeFormatted = result.criticalValueFreeze !== null ? 
        result.criticalValueFreeze.toFixed(2) + ' ' + result.paramDef.unit : 
        outOfRangeText;
      
      const safetyFormatted = result.criticalValueSafety !== null ? 
