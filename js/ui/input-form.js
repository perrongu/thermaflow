...
(function() {
  ...
  function waitForSVGInputs() {
    const inputIds = [
      'pipe-length', 'water-temp', 'water-flow', 'water-pressure',
      'air-temp', 'wind-speed', 'flow-unit', 'pressure-unit'
    ];
  ...
  function updateScheduleLabel(material) {
    if (PipeSpecsLoader.usesTypes(material)) {
      ...
      elements.pipeScheduleLabel.textContent = (window.I18n ? I18n.t('controls.schedule') : 'Schedule');
    }
  }
  ...
  function updatePipeSpecs() {
    ...
    if (!material || !schedule || !nps || isNaN(nps)) {
      console.warn('Paramètres de tuyau incomplets ou invalides:', { material, schedule, nps });
      return;
    }
  ...
  function validateForm() {
    ...
    for (const field of requiredFields) {
      if (!field.value || field.value === '') {
        ...
        const msg = window.I18n ? I18n.t('validation.requiredMissing', { label }) : `Champ requis manquant: ${label}`;
        alert(msg);
        field.focus();
        return false;
      }
    ...
    if (elements.pipeLength.value < 1 || elements.pipeLength.value > 1000) {
      alert(window.I18n ? I18n.t('validation.lengthRange') : 'Longueur doit être entre 1 et 1000 m');
      elements.pipeLength.focus();
      return false;
    }

    if (elements.waterTemp.value < 1 || elements.waterTemp.value > 100) {
      alert(window.I18n ? I18n.t('validation.waterTempRange') : 'Température eau doit être entre 1 et 100°C');
      elements.waterTemp.focus();
      return false;
    }

    if (elements.airTemp.value < -50 || elements.airTemp.value > 30) {
      alert(window.I18n ? I18n.t('validation.airTempRange') : 'Température air doit être entre -50 et 30°C');
    ...
    if (pressureValue < pressureRanges.min || pressureValue > pressureRanges.max) {
      ...
      const msg = window.I18n 
        ? I18n.t('validation.waterPressureRange').replace('100', pressureRanges.min.toFixed(0)).replace('1000', pressureRanges.max.toFixed(0)).replace('kPag', pressureUnit)
    ...
    if (flowValue < flowRanges.min || flowValue > flowRanges.max) {
      ...
      const msg = window.I18n 
        ? I18n.t('validation.waterFlowRange').replace('0.06', flowRanges.min.toFixed(2)).replace('30', flowRanges.max.toFixed(2)).replace('m³/hr', flowUnit)
    ...
    if (elements.windSpeed.value < 0 || elements.windSpeed.value > 108) {
      alert(window.I18n ? I18n.t('validation.windSpeedRange') : 'Vitesse vent doit être entre 0 et 108 km/h');
  ...
  function handleUnitChange(paramType, newUnit, inputId) {
    ...
    console.log(`🔄 Unité changée: ${paramType} ${oldUnit} → ${newUnit}, valeur: ${currentValue} → ${convertedValue.toFixed(2)}`);
  }
