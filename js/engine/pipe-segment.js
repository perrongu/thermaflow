...
function calculatePipeSegment(geometry, fluid, ambient, insulation = null, iterations = 2) {
  ...
  if (typeof iterations !== 'number' || !isFinite(iterations) || iterations < 1 || iterations > 10) {
  ...
  const waterProps = typeof window !== 'undefined' ? window.WaterProperties : require('../properties/water-properties.js');
  const airProps = typeof window !== 'undefined' ? window.AirProperties : require('../properties/air-properties.js');
  const materials = typeof window !== 'undefined' ? window.MaterialProperties : require('../properties/material-properties.js');
  const reynolds = typeof window !== 'undefined' ? window.Reynolds : require('../formulas/reynolds.js');
  const geom = typeof window !== 'undefined' ? window.Geometry : require('../formulas/geometry.js');
  const pressureBasic = typeof window !== 'undefined' ? window.PressureBasic : require('../formulas/pressure-basic.js');
  const friction = typeof window !== 'undefined' ? window.FrictionFactor : require('../correlations/friction-factor.js');
  const nusseltInt = typeof window !== 'undefined' ? window.NusseltInternal : require('../correlations/nusselt-internal.js');
  const nusseltExt = typeof window !== 'undefined' ? window.NusseltExternal : require('../correlations/nusselt-external.js');
  const radiation = typeof window !== 'undefined' ? window.Radiation : require('../correlations/radiation.js');
  const resistance = typeof window !== 'undefined' ? window.ThermalResistance : require('../calculations/thermal-resistance.js');
  const heatTransfer = typeof window !== 'undefined' ? window.HeatTransfer : require('../calculations/heat-transfer.js');
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    const h_rad = radiation.radiationCoefficientSimple(fluid.T_in, ambient.T_amb, pipeMat.emissivity);
