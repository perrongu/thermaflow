...
function calculatePipeNetwork(config) {
  ...
  if (typeof config.numSegments !== 'number' || config.numSegments < 1 || !Number.isInteger(config.numSegments)) {
  ...
  // ========== MODULE NÉCESSAIRE ==========
  const pipeSegment = typeof window !== 'undefined' 
  ...
  for (let i = 0; i < config.numSegments; i++) {
    ...
    try {
      ...
      if (error.message && error.message.includes('Température hors plage') && currentFluid.T_in <= 0) {
...
function interpolateTemperature(x_profile, T_profile, x) {
  if (x < x_profile[0] || x > x_profile[x_profile.length - 1]) {
    throw new Error(`Position hors plage: ${x} (plage: ${x_profile[0]}-${x_profile[x_profile.length - 1]})`);
  }
  ...
  for (let i = 0; i < x_profile.length - 1; i++) {
    if (x >= x_profile[i] && x <= x_profile[i + 1]) {
      ...
      if (x2 === x1) return T1;  // Éviter division par zéro
