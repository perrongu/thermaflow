...
function generateCriticalCases() {
  ...
  const extremeTempCases = [
    ...
    { temp: 100, ambient: -50, desc: 'Température eau maximale, air très froid' },
  ...
  const freezeCases = [
    { temp: 2, ambient: -30, wind: 0, insul: null, desc: 'Risque gel élevé sans isolation' },
    { temp: 2, ambient: -30, wind: 50, insul: null, desc: 'Risque gel élevé avec vent sans isolation' },
    { temp: 2, ambient: -30, wind: 0, insul: { material: 'polyurethane_foam', thickness_mm: 50 }, desc: 'Risque gel avec isolation épaisse' },
    { temp: 5, ambient: -40, wind: 30, insul: { material: 'fiberglass', thickness_mm: 25 }, desc: 'Risque gel modéré avec isolation moyenne' }
  ];
  ...
  const geomCases = [
    { material: 'steel', schedule: '40', nps: 0.5, length: 1, desc: 'Acier petit diamètre, longueur minimale' },
    { material: 'steel', schedule: '160', nps: 36, length: 1000, desc: 'Acier grand diamètre, longueur maximale' },
    { material: 'copper', schedule: 'K', nps: 0.25, length: 10, desc: 'Cuivre petit diamètre' },
    { material: 'copper', schedule: 'M', nps: 12, length: 500, desc: 'Cuivre grand diamètre' },
    { material: 'stainless_steel', schedule: '10S', nps: 0.125, length: 5, desc: 'Inox petit diamètre' },
    { material: 'stainless_steel', schedule: '80S', nps: 24, length: 800, desc: 'Inox grand diamètre' }
  ];
  ...
  const insulCases = [
    { material: 'fiberglass', thickness: 13, desc: 'Isolation minimale fibre de verre' },
    { material: 'polyurethane_foam', thickness: 100, desc: 'Isolation maximale polyuréthane' },
    { material: 'mineral_wool', thickness: 50, desc: 'Isolation moyenne laine minérale' },
    { material: 'elastomeric_foam', thickness: 25, desc: 'Isolation élastomère' }
  ];
...
function generateLHSCases(n, startId) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const commonNPS = [0.125, 0.25, 0.375, 0.5, 0.75, 1, 1.25, 1.5, 2, 2.5, 3, 4, 6, 8, 10, 12, 14, 16, 18, 20, 24, 30, 36];
...
function validateCase(testCase) {
  ...
  if (fluid.pressure_kPag < CONFIG.ranges.pressure_kPag.min || fluid.pressure_kPag > CONFIG.ranges.pressure_kPag.max) {
    errors.push(`Pression hors plage: ${fluid.pressure_kPag}`);
  }
  
  if (ambient.temp_C < CONFIG.ranges.ambient_temp_C.min || ambient.temp_C > CONFIG.ranges.ambient_temp_C.max) {
    errors.push(`Température air hors plage: ${ambient.temp_C}`);
  }
  
  if (ambient.wind_kmh < CONFIG.ranges.wind_kmh.min || ambient.wind_kmh > CONFIG.ranges.wind_kmh.max) {
  ...
  if (insulation !== null) {
    ...
    if (insulation.thickness_mm < CONFIG.insulationThickness.min || insulation.thickness_mm > CONFIG.insulationThickness.max) {
...
function displayStatistics(cases) {
  ...
  for (const testCase of cases) {
    ...
    if (insulation === null) {
      ...
      insulMaterialCounts[insulation.material] = (insulMaterialCounts[insulation.material] || 0) + 1;
  ...
  console.log('\n📏 Catégories NPS:');
  console.log(`   Petit (≤2")          : ${npsCounts.small.toString().padStart(3)} (${(npsCounts.small/cases.length*100).toFixed(1)}%)`);
  console.log(`   Moyen (2-8")         : ${npsCounts.medium.toString().padStart(3)} (${(npsCounts.medium/cases.length*100).toFixed(1)}%)`);
  console.log(`   Grand (>8")          : ${npsCounts.large.toString().padStart(3)} (${(npsCounts.large/cases.length*100).toFixed(1)}%)`);
  
  console.log('\n🧊 Isolation:');
  console.log(`   Sans isolation       : ${insulationCounts.none.toString().padStart(3)} (${(insulationCounts.none/cases.length*100).toFixed(1)}%)`);
  console.log(`   Avec isolation       : ${insulationCounts.with.toString().padStart(3)} (${(insulationCounts.with/cases.length*100).toFixed(1)}%)`);
  ...
  const stats = (values, name, unit) => {
    ...
    console.log(`   ${name.padEnd(25)} : ${min.toFixed(1).padStart(7)} / ${median.toFixed(1).padStart(7)} / ${max.toFixed(1).padStart(7)} ${unit}`);
  };
