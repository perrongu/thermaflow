...
function loadThermaFlowModules(rootDir) {
  ...
  const { materialPropertiesData } = require(
    path.join(rootDir, 'data', 'materials', 'properties.js')
  );
...
function loadPipeSpecsHelper(rootDir) {
  ...
  const copperCodeModified = copperCode.replace(
    /^const copperData =/m,
  ...
  const stainlessCode = fs.readFileSync(
    path.join(rootDir, 'data', 'pipespecs', 'stainless_steel.js'),
    'utf8'
  );
  const stainlessCodeModified = stainlessCode.replace(
  ...
  function getPipeSpecs(material, scheduleOrType, nps) {
    let data = null;
    const key = String(scheduleOrType);
