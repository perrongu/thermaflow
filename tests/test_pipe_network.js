...
function assertApprox(actual, expected, tolerance, message) {
  ...
  if (diff <= tolerance || relError <= tolerance) {
    ...
    console.error(`  ❌ ÉCHEC: ${message}`);
    console.error(
      `     Attendu: ${expected}, Obtenu: ${actual}, Différence: ${diff}`,
    );
  }
}
...
try {
  ...
  assert(
    false,
    `Configuration valide ne devrait pas lever d'erreur: ${e.message}`,
  );
}
...
assert(
  result1.T_profile.length === baseConfig.numSegments + 1,
  "T_profile devrait avoir N+1 valeurs",
);
assert(
  result1.x_profile.length === baseConfig.numSegments + 1,
  "x_profile devrait avoir N+1 valeurs",
);
assert(
  result1.P_profile.length === baseConfig.numSegments + 1,
  "P_profile devrait avoir N+1 valeurs",
);
assert(
  result1.segmentResults.length === baseConfig.numSegments,
  "segmentResults devrait avoir N valeurs",
);

// Test 2.2: Cohérence des profils
assert(
  result1.T_profile[0] === baseConfig.fluid.T_in,
  "Première température devrait être T_in",
);
assert(
  result1.T_profile[result1.T_profile.length - 1] === result1.T_final,
  "Dernière température devrait être T_final",
);
assert(result1.x_profile[0] === 0, "Position initiale devrait être 0");
assertApprox(
  result1.x_profile[result1.x_profile.length - 1],
  baseConfig.totalLength,
  1e-6,
  "Position finale devrait être totalLength",
);
assert(
  result1.P_profile[0] === baseConfig.fluid.P,
...
assert(
  temperatureMonotonic,
  "Température devrait être décroissante (refroidissement)",
);
...
assert(
  pressureMonotonic,
  "Pression devrait être décroissante (perte de charge)",
);
...
assert(
  result1.Q_loss_total > 0,
  "Perte thermique totale devrait être positive",
);

// Test 3.5: Température minimale
assert(
  result1.minTemp <= baseConfig.fluid.T_in,
  "Température minimale devrait être ≤ T_in",
);
assert(
  result1.minTemp >= baseConfig.ambient.T_amb,
  "Température minimale devrait être ≥ T_amb",
);
assert(
  result1.minTempPosition >= 0 &&
    result1.minTempPosition <= baseConfig.totalLength,
  "Position température minimale devrait être dans [0, L]",
);

console.log(
  `  ℹ️  T_in = ${baseConfig.fluid.T_in}°C → T_final = ${result1.T_final.toFixed(2)}°C`,
);
console.log(
  `  ℹ️  Température min = ${result1.minTemp.toFixed(2)}°C à ${result1.minTempPosition}m`,
);
console.log(
  `  ℹ️  Q_loss_total = ${(result1.Q_loss_total / 1000).toFixed(2)} kW`,
);
...
console.log(
  `  ℹ️  10 segments:  T_final = ${result10seg.T_final.toFixed(2)}°C`,
);
console.log(
  `  ℹ️  100 segments: T_final = ${result100seg.T_final.toFixed(2)}°C`,
);
...
assertApprox(
  dP_sum_from_segments,
  result10seg.dP_total,
  1e-6,
  "dP_total devrait être la somme des dP des segments",
);

console.log(
  `  ℹ️  Somme Q_loss segments: ${(Q_sum_from_segments / 1000).toFixed(2)} kW`,
);
console.log(
  `  ℹ️  Q_loss_total:         ${(result10seg.Q_loss_total / 1000).toFixed(2)} kW`,
);
console.log(
  `  ℹ️  Somme dP segments:     ${(dP_sum_from_segments / 1000).toFixed(1)} kPa`,
);
console.log(
  `  ℹ️  dP_total:              ${(result10seg.dP_total / 1000).toFixed(1)} kPa`,
);
...
for (let i = 0; i < result10seg.segmentResults.length - 1; i++) {
  ...
  if (Math.abs(T_out_i - T_in_next) > 1e-9) {
    ...
    console.error(
      `    Segment ${i}: T_out = ${T_out_i}, T_in[i+1] = ${T_in_next}`,
    );
    break;
  }
}
assert(
  propagationCorrect,
  "T_out du segment i devrait égaler T_in du segment i+1",
);
...
assert(
  seg5 === 4 || seg5 === 5,
  "Position 50 devrait être dans segment 4 ou 5 (à l'interface)",
);
assert(seg10 === 9, "Position 100 devrait être dans dernier segment");

// Test 7.2: interpolateTemperature
const T_at_0 = pipeNetwork.interpolateTemperature(
  result10seg.x_profile,
  result10seg.T_profile,
  0,
);
const T_at_50 = pipeNetwork.interpolateTemperature(
  result10seg.x_profile,
  result10seg.T_profile,
  50,
);
const T_at_100 = pipeNetwork.interpolateTemperature(
  result10seg.x_profile,
  result10seg.T_profile,
  100,
);
...
assertApprox(
  T_at_0,
  result10seg.T_profile[0],
  1e-9,
  "Température interpolée à x=0 devrait être T_profile[0]",
);
assertApprox(
  T_at_100,
  result10seg.T_profile[result10seg.T_profile.length - 1],
  1e-9,
  "Température interpolée à x=L devrait être T_profile[N]",
);
assert(
  T_at_50 > result10seg.T_profile[result10seg.T_profile.length - 1],
  "Température à mi-chemin devrait être > T_final",
);
assert(
  T_at_50 < result10seg.T_profile[0],
...
assertThrows(() => {
  pipeNetwork.interpolateTemperature(
    result10seg.x_profile,
    result10seg.T_profile,
    -10,
  );
...
assert(
  resultNoInsul.Q_loss_total > resultWithInsul.Q_loss_total,
  "Sans isolation devrait perdre plus de chaleur",
);
assert(
  resultNoInsul.T_final < resultWithInsul.T_final,
  "Sans isolation devrait avoir T_final plus basse",
);

console.log(
  `  ℹ️  Sans isolation: T_final = ${resultNoInsul.T_final.toFixed(2)}°C, Q_loss = ${(resultNoInsul.Q_loss_total / 1000).toFixed(2)} kW`,
);
console.log(
  `  ℹ️  Avec isolation: T_final = ${resultWithInsul.T_final.toFixed(2)}°C, Q_loss = ${(resultWithInsul.Q_loss_total / 1000).toFixed(2)} kW`,
);
...
const configLong = {
  ...
  totalLength: 2500,
  numSegments: 50,
  fluid: { ...baseConfig.fluid, P: 6.0 },
};
const resultLong = pipeNetwork.calculatePipeNetwork(configLong);
assert(
  resultLong.T_final < result10seg.T_final,
...
assert(
  resultLowFlow.T_final < result10seg.T_final,
...
assert(
  resultColdAir.T_final < result10seg.T_final,
...
console.log(
  `  ℹ️  Long (2500m):                  T_final = ${resultLong.T_final.toFixed(2)}°C`,
);
console.log(
  `  ℹ️  Débit faible (0.5 kg/s):       T_final = ${resultLowFlow.T_final.toFixed(2)}°C`,
);
console.log(
