...
assert(result1.T_profile[0] === baseConfig.fluid.T_in, 'Première température devrait être T_in');
assert(
...
assert(result1.P_profile[0] === baseConfig.fluid.P, 'Pression initiale devrait être P');
...
assert(result1.minTemp <= baseConfig.fluid.T_in, 'Température minimale devrait être ≤ T_in');
assert(result1.minTemp >= baseConfig.ambient.T_amb, 'Température minimale devrait être ≥ T_amb');
assert(
  result1.minTempPosition >= 0 && result1.minTempPosition <= baseConfig.totalLength,
...
for (const seg of result10seg.segmentResults) {
...
assert(T_at_50 < result10seg.T_profile[0], 'Température à mi-chemin devrait être < T_in');
...
assert(resultLowFlow.T_final < result10seg.T_final, 'Débit faible devrait refroidir davantage');
...
assert(resultColdAir.T_final < result10seg.T_final, 'Air très froid devrait refroidir davantage');
...
assert(resultExtreme.T_final < configExtreme.fluid.T_in, 'Cas extrême devrait refroidir');
...
console.log(`  ℹ️  Long (2500m):                  T_final = ${resultLong.T_final.toFixed(2)}°C`);
