...
const result_1iter_low = pipeSegment.calculatePipeSegment(geometry, fluid_low, ambient, insulation, 1);
const result_2iter_low = pipeSegment.calculatePipeSegment(geometry, fluid_low, ambient, insulation, 2);
...
assertLess(rel_diff_low, 0.02, `ΔT faible: différence < 2% (obtenu ${(rel_diff_low*100).toFixed(2)}%)`);
...
const result_1iter_med = pipeSegment.calculatePipeSegment(geometry_long, fluid_med, ambient, null, 1);  // Sans isolation
const result_2iter_med = pipeSegment.calculatePipeSegment(geometry_long, fluid_med, ambient, null, 2);
...
assertGreater(rel_diff_med, 0.0001, `ΔT moyen: amélioration > 0.01% (obtenu ${(rel_diff_med*100).toFixed(3)}%)`);
assertLess(rel_diff_med, 0.08, `ΔT moyen: amélioration < 8% (obtenu ${(rel_diff_med*100).toFixed(3)}%)`);
...
const result_1iter_high = pipeSegment.calculatePipeSegment(geometry_verylong, fluid_high, ambient, null, 1);
const result_2iter_high = pipeSegment.calculatePipeSegment(geometry_verylong, fluid_high, ambient, null, 2);
...
assertGreater(rel_diff_high, 0.001, `ΔT élevé: amélioration > 0.1% (obtenu ${(rel_diff_high*100).toFixed(3)}%)`);
assertLess(rel_diff_high, 0.15, `ΔT élevé: amélioration < 15% (obtenu ${(rel_diff_high*100).toFixed(3)}%)`);

console.log('\nTest 4: Convergence 2 itérations vs 5 itérations (< 1% écart)');
const result_2iter = pipeSegment.calculatePipeSegment(geometry_verylong, fluid_high, ambient, null, 2);
const result_5iter = pipeSegment.calculatePipeSegment(geometry_verylong, fluid_high, ambient, null, 5);
...
assertLess(rel_diff_2_5, 0.01, `Convergence 2→5: < 1% écart (obtenu ${(rel_diff_2_5*100).toFixed(3)}%)`);

console.log('\nTest 5: Conservation énergie avec itération');
// Q_loss = ṁ·cp·(T_in - T_out) doit être cohérent
const result_conv = pipeSegment.calculatePipeSegment(geometry_verylong, fluid_high, ambient, null, 2);
...
assertClose(result_conv.Q_loss, Q_verify, 0.15, `Conservation: Q_loss cohérent (${result_conv.Q_loss.toFixed(0)}W vs ${Q_verify.toFixed(0)}W)`);
...
const result_default = pipeSegment.calculatePipeSegment(geometry, fluid_compat, ambient, insulation);
const result_explicit2 = pipeSegment.calculatePipeSegment(geometry, fluid_compat, ambient, insulation, 2);

assertClose(result_default.T_out, result_explicit2.T_out, 0.001, 'Défaut = 2 itérations explicites');

// 1 itération vs 2 itérations avec pertes significatives
const result_v1_noinsul = pipeSegment.calculatePipeSegment(geometry_long, fluid_med, ambient, null, 1);
const result_v2_noinsul = pipeSegment.calculatePipeSegment(geometry_long, fluid_med, ambient, null, 2);
testsRun++;
...
const result_1iter_ext = pipeSegment.calculatePipeSegment(geometry_extreme, fluid_extreme, ambient_warm, null, 1);  // Sans isolation
const result_2iter_ext = pipeSegment.calculatePipeSegment(geometry_extreme, fluid_extreme, ambient_warm, null, 2);
...
assertGreater(rel_diff_ext, 0.0001, `Cas extrême: amélioration > 0.01% (obtenu ${(rel_diff_ext*100).toFixed(3)}%)`);
