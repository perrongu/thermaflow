...
assertLess(diff_2299_2301, 0.05, `Continuité Re=2299→2301: ${(diff_2299_2301*100).toFixed(2)}% < 5%`);
...
assertEqual(regime_4000, 'transitional', 'Re=4000 → régime transitoire (limite haute)');
...
assertLess(diff_3999_4001, 0.05, `Continuité Re=3999→4001: ${(diff_3999_4001*100).toFixed(2)}% < 5%`);
...
assertClose(Nu_gnielinski, Nu_dittus, 0.10, `Gnielinski ≈ Dittus-Boelter: ${Nu_gnielinski.toFixed(1)} ≈ ${Nu_dittus.toFixed(1)}`);
...
assertClose(f_low, f_poiseuille, 0.01, `f(Re=100) = ${f_low.toFixed(3)} ≈ 64/Re = ${f_poiseuille.toFixed(3)}`);
...
const L_hausen = 10;  // m
const Nu_Hausen_06 = nusseltInt.nusseltHausen(Re_hausen, 0.6, D_hausen, L_hausen);
assertGreater(Nu_Hausen_06, 4, `Nu Hausen(Pr=0.6) = ${Nu_Hausen_06.toFixed(1)} > 4`);
...
assertGreater(air_props.Pr, 0.70, `Pr air(20°C) = ${air_props.Pr.toFixed(3)} > 0.70`);
assertLess(air_props.Pr, 0.73, `Pr air(20°C) = ${air_props.Pr.toFixed(3)} < 0.73`);
...
assertGreater(Pr_water_calc, 5, `Pr eau(20°C) = ${Pr_water_calc.toFixed(2)} > 5`);
...
assertGreater(water_0C.rho, 995, `ρ eau(0°C) = ${water_0C.rho.toFixed(1)} > 995 kg/m³`);
assertLess(water_0C.rho, 1005, `ρ eau(0°C) = ${water_0C.rho.toFixed(1)} < 1005 kg/m³`);

console.log('\nTest C2: Eau T = 100°C exact (limite haute)');
const water_100C = waterProps.getWaterProperties(100, 3.0);
assertGreater(water_100C.rho, 950, `ρ eau(100°C) = ${water_100C.rho.toFixed(1)} > 950 kg/m³`);
assertLess(water_100C.rho, 970, `ρ eau(100°C) = ${water_100C.rho.toFixed(1)} < 970 kg/m³`);

console.log('\nTest C3: Eau T = -1°C (erreur attendue)');
assertThrows(() => waterProps.getWaterProperties(-1, 3.0), 'Eau T=-1°C → erreur');

console.log('\nTest C4: Eau T = 101°C (erreur attendue)');
assertThrows(() => waterProps.getWaterProperties(101, 3.0), 'Eau T=101°C → erreur');

console.log('\nTest C5: Air T = -40°C exact (limite basse)');
const air_minus40 = airProps.getAirProperties(-40);
assertGreater(air_minus40.rho, 1.4, `ρ air(-40°C) = ${air_minus40.rho.toFixed(3)} > 1.4 kg/m³`);
assertLess(air_minus40.rho, 1.6, `ρ air(-40°C) = ${air_minus40.rho.toFixed(3)} < 1.6 kg/m³`);

console.log('\nTest C6: Air T = 50°C exact (limite haute)');
const air_50 = airProps.getAirProperties(50);
assertGreater(air_50.rho, 1.0, `ρ air(50°C) = ${air_50.rho.toFixed(3)} > 1.0 kg/m³`);
assertLess(air_50.rho, 1.15, `ρ air(50°C) = ${air_50.rho.toFixed(3)} < 1.15 kg/m³`);
...
assertGreater(f_std, 0.020, `f(ε/D=0.001) = ${f_std.toFixed(5)} > 0.020`);
assertLess(f_std, 0.030, `f(ε/D=0.001) = ${f_std.toFixed(5)} < 0.030`);

console.log('\nTest D2: ε/D = 0.01 (rugueux)');
const f_rough = friction.frictionFactorChurchill(50000, 0.01);
assertGreater(f_rough, 0.030, `f(ε/D=0.01) = ${f_rough.toFixed(5)} > 0.030`);
assertLess(f_rough, 0.050, `f(ε/D=0.01) = ${f_rough.toFixed(5)} < 0.050`);

console.log('\nTest D3: ε/D = 0.05 (limite haute Colebrook)');
const f_very_rough = friction.frictionFactorColebrook(50000, 0.05);
assertGreater(f_very_rough, 0.050, `f(ε/D=0.05) = ${f_very_rough.toFixed(5)} > 0.050`);
assertLess(f_very_rough, 0.090, `f(ε/D=0.05) = ${f_very_rough.toFixed(5)} < 0.090`);
...
assertGreater(f_smooth, 0.010, `f(ε/D=0) = ${f_smooth.toFixed(5)} > 0.010`);
...
assertGreater(delta_rho, 0.02, `Δρ eau(10→90°C) = ${(delta_rho*100).toFixed(1)}% > 2%`);

console.log('\nTest E2: μ eau varie avec T');
const delta_mu = Math.abs(water_90.mu - water_10.mu) / water_10.mu;

assertGreater(delta_mu, 0.30, `Δμ eau(10→90°C) = ${(delta_mu*100).toFixed(1)}% > 30%`);

console.log('\nTest E3: cp eau quasi-constant');
const delta_cp = Math.abs(water_90.cp - water_10.cp) / water_10.cp;

assertLess(delta_cp, 0.02, `Δcp eau(10→90°C) = ${(delta_cp*100).toFixed(1)}% < 2%`);
...
assertGreater(f_5k, f_50k, `f(Re=5k) = ${f_5k.toFixed(4)} > f(Re=50k) = ${f_50k.toFixed(4)}`);
...
assertGreater(Nu_50k, Nu_5k, `Nu(Re=50k) = ${Nu_50k.toFixed(1)} > Nu(Re=5k) = ${Nu_5k.toFixed(1)}`);
...
assertGreater(Nu_Pr10, Nu_Pr1, `Nu(Pr=10) = ${Nu_Pr10.toFixed(1)} > Nu(Pr=1) = ${Nu_Pr1.toFixed(1)}`);
...
assertClose(f_lam, f_poiseuille_exact, 0.001, `f(Re=1000) = ${f_lam.toFixed(4)} ≈ 64/Re = ${f_poiseuille_exact.toFixed(4)}`);
...
assertClose(Nu_hausen_dev, 3.66, 0.10, `Nu développé = ${Nu_hausen_dev.toFixed(2)} ≈ 3.66`);
...
assertLess(err_rad_50K, 0.03, `Erreur radiation ΔT=50K: ${(err_rad_50K*100).toFixed(2)}% < 3%`);
...
assertLess(err_rad_100K, 0.10, `Erreur radiation ΔT=100K: ${(err_rad_100K*100).toFixed(2)}% < 10% (excellent!)`);
...
assertClose(air_20.rho, rho_ideal, 0.005, `ρ air(20°C) = ${air_20.rho.toFixed(3)} ≈ P/(RT) = ${rho_ideal.toFixed(3)}`);
...
assertGreater(Nu_short, Nu_long, `Nu(L=0.1m) = ${Nu_short.toFixed(1)} > Nu(L=10m) = ${Nu_long.toFixed(1)}`);

console.log('\nTest G2: Longueur très longue (L = 1000m, développé)');
// Nu devrait tendre vers valeur développée
const Nu_verylong = nusseltInt.nusseltHausen(5000, Pr_water, 0.05, 1000);

assertClose(Nu_verylong, 3.66, 0.15, `Nu(L=1000m) = ${Nu_verylong.toFixed(2)} ≈ 3.66 (développé)`);
...
assertGreater(air_extreme.mu, 1.4e-5, `μ air(-40°C) = ${air_extreme.mu.toExponential(2)} > 1.4e-5`);
assertGreater(air_extreme.k, 0.020, `k air(-40°C) = ${air_extreme.k.toFixed(4)} > 0.020`);
...
assertClose(f_verylow, 64/Re_verylow, 0.01, `f(Re=500) = ${f_verylow.toFixed(4)} ≈ 64/Re = ${(64/Re_verylow).toFixed(4)}`);
...
assertGreater(Re_vhigh, 200000, `Re(V=5m/s) = ${Re_vhigh.toExponential(2)} > 200000`);
...
assertGreater(water_10bar.rho, 980, `ρ eau(50°C, 10bar) = ${water_10bar.rho.toFixed(1)} > 980`);
assertLess(water_10bar.rho, 1000, `ρ eau(50°C, 10bar) = ${water_10bar.rho.toFixed(1)} < 1000`);
