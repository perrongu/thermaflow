...
assertGreater(air_props.Pr, 0.7, `Pr air(20°C) = ${air_props.Pr.toFixed(3)} > 0.70`);
...
assertGreater(f_very_rough, 0.05, `f(ε/D=0.05) = ${f_very_rough.toFixed(5)} > 0.050`);
assertLess(f_very_rough, 0.09, `f(ε/D=0.05) = ${f_very_rough.toFixed(5)} < 0.090`);
...
assertGreater(delta_mu, 0.3, `Δμ eau(10→90°C) = ${(delta_mu * 100).toFixed(1)}% > 30%`);
...
assertClose(Nu_hausen_dev, 3.66, 0.1, `Nu développé = ${Nu_hausen_dev.toFixed(2)} ≈ 3.66`);
...
assertClose(Nu_verylong, 3.66, 0.15, `Nu(L=2500m) = ${Nu_verylong.toFixed(2)} ≈ 3.66 (développé)`);
...
assertGreater(air_extreme.k, 0.02, `k air(-40°C) = ${air_extreme.k.toFixed(4)} > 0.020`);
