 ...
 */
function extractConstantFromCode(filePath, constantPattern, _hints = {}) {
  try {
...
function validateConstants(references) {
  ...
  for (const [name, ref] of Object.entries(constants)) {
    ...
    if (withinTolerance) {
      ...
      console.log(
        `  ${colors.green}✓${colors.reset} ${name}: ${extracted.value} (attendu: ${ref.value})`
      );
    } else {
      results.constants.fail++;
      ...
      console.log(
        `  ${colors.red}✗${colors.reset} ${name}: ${extracted.value} (attendu: ${ref.value}, écart: ${diff})`
      );
    }
  }
  ...
  for (const [name, conv] of Object.entries(conversions)) {
    ...
    for (const testCase of conv.test_cases) {
      ...
      if (pass) {
        ...
        console.log(
          `    ${colors.green}✓${colors.reset} ${testCase.input} → ${result.toExponential(4)}`
        );
      } else {
        results.conversions.fail++;
        results.conversions.warnings.push(
          `${name}: ${testCase.input} → ${result} (attendu ${testCase.expected})`
        );
        console.log(
          `    ${colors.red}✗${colors.reset} ${testCase.input} → ${result} (attendu ${testCase.expected})`
        );
      }
...
const VALIDATION_FILE = path.join(ROOT_DIR, 'validation', 'external_validation_sample_v1.0.1.json');
...
function processExternalValidation() {
  ...
  if (!fs.existsSync(VALIDATION_FILE)) {
    console.log(
      `  ${colors.yellow}Fichier de validation externe non trouvé, passage...${colors.reset}`
    );
    return null;
  }
  ...
  if (!fs.existsSync(backupFile)) {
    try {
      ...
      console.log(
        `  ${colors.yellow}⚠ Impossible de créer backup: ${error.message}${colors.reset}`
      );
    }
  }
  ...
  try {
    ...
    console.log(
      `  ${colors.yellow}Utilisation des résultats ThermaFlow existants...${colors.reset}`
    );
    modules = null;
  }
  ...
  if (modules && pipeSpecsHelper) {
    ...
    console.log(
      `\r  ${colors.green}✓${colors.reset} ${recalculated} cas recalculés (${recalcErrors} erreurs)          `
    );
  ...
  for (const testCase of validationData.cases) {
    ...
    if (hysys.status === 'ok') {
      results.externalValidation.hysysCases++;
    }
    if (aft.status === 'ok') {
      results.externalValidation.aftCases++;
    }
    if (dwsim.status === 'ok') {
      results.externalValidation.dwsimCases++;
    }
    ...
    if (hysys.status === 'ok' && hysys.T_out_C !== null) {
      ...
      if (hysys.heat_loss_W !== null && thermaflow.heat_loss_W !== 0) {
        const devQ = Math.abs(
          ((thermaflow.heat_loss_W - hysys.heat_loss_W) / thermaflow.heat_loss_W) * 100
        );
    ...
    if (aft.status === 'ok' && aft.T_out_C !== null) {
      ...
      if (aft.heat_loss_W !== null && thermaflow.heat_loss_W !== 0) {
        const devQ = Math.abs(
          ((thermaflow.heat_loss_W - aft.heat_loss_W) / thermaflow.heat_loss_W) * 100
        );
    ...
    if (dwsim.status === 'ok' && dwsim.T_out_C !== null) {
      ...
      if (dwsim.heat_loss_W !== null && thermaflow.heat_loss_W !== 0) {
        const devQ = Math.abs(
          ((thermaflow.heat_loss_W - dwsim.heat_loss_W) / thermaflow.heat_loss_W) * 100
        );
    ...
    if (validPressures.length > 0) {
      ...
      if (
        devP > VALIDATION_THRESHOLDS.PRESSURE_DEVIATION_KPA &&
  ...
  console.log(
    `  Cas testés: ${results.externalValidation.testedCases}/${results.externalValidation.totalCases}`
  );
...
function runTests() {
  ...
  const testFiles = fs
    .readdirSync(path.join(ROOT_DIR, 'tests'))
...
function generateExternalValidationSection() {
  ...
  if (results.externalValidation.hysysCases > 0) {
    softwareCounts.push(`${results.externalValidation.hysysCases} Hysys`);
  }
  if (results.externalValidation.aftCases > 0) {
    softwareCounts.push(`${results.externalValidation.aftCases} AFT`);
  }
  if (results.externalValidation.dwsimCases > 0) {
    softwareCounts.push(`${results.externalValidation.dwsimCases} DWSIM`);
  }
  ...
  if (consolidated.T_out && consolidated.T_out.mean < 1.5) {
    ...
    consolidated.T_out &&
    consolidated.T_out.mean < VALIDATION_THRESHOLDS.TEMP_DEVIATION_C
  ) {
...
`;
  ...
  if (canSign) {
    ...
    report += `**Je certifie l'exactitude scientifique et technique de ThermaFlow v${VERSION}**\n\n`;
  } else {
  ...
  report += `*ThermaFlow v${VERSION} - Automated Verification System*\n`;
...
function main() {
  ...
  console.log(
    `${colors.blue}║    THERMAFLOW - VÉRIFICATION AUTOMATIQUE                 ║${colors.reset}`
  );
  console.log(
    `${colors.blue}╚═══════════════════════════════════════════════════════════╝${colors.reset}\n`
  );
  ...
  try {
    ...
    console.log(
      `${colors.green}✓${colors.reset} Références chargées: ${Object.keys(references.physical_constants).length} constantes, ${Object.keys(references.unit_conversions).length} conversions`
    );
    ...
    console.log(
      `Constantes : ${results.constants.pass}/${results.constants.total} ` +
        (results.constants.fail === 0 ? colors.green + '✓' : colors.red + '✗') +
        colors.reset
    );
    console.log(
      `Conversions: ${results.conversions.pass}/${results.conversions.total} ` +
        (results.conversions.fail === 0 ? colors.green + '✓' : colors.red + '✗') +
        colors.reset
    );
    console.log(
      `Tests      : ${results.tests.pass}/${results.tests.total} ` +
        (results.tests.fail === 0 ? colors.green + '✓' : colors.red + '✗') +
        colors.reset
    );

    if (results.externalValidation.totalCases > 0) {
      console.log(
        `Validation : ${results.externalValidation.testedCases}/${results.externalValidation.totalCases} cas ` +
          colors.green +
          '✓' +
          colors.reset
      );
