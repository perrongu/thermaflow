...
assert(
  result1.T_out > validAmbient.T_amb,
  "T_out devrait être > T_amb (ne peut pas être plus froid que l'air)"
);
...
assert(
  ['laminar', 'transitional', 'turbulent'].includes(result1.regime),
...
const resultNoInsul = pipeSegment.calculatePipeSegment(
  validGeometry,
  validFluid,
  validAmbient,
  null
...
const resultWithInsul = pipeSegment.calculatePipeSegment(
  validGeometry,
  validFluid,
  validAmbient,
  validInsulation
...
assert(resultWithInsul.Q_loss < resultNoInsul.Q_loss, 'Isolation devrait réduire Q_loss');

// L'isolation devrait augmenter T_out (moins de refroidissement)
assert(resultWithInsul.T_out > resultNoInsul.T_out, 'Isolation devrait augmenter T_out');

// L'isolation devrait augmenter la résistance thermique
assert(resultWithInsul.R_total > resultNoInsul.R_total, 'Isolation devrait augmenter R_total');
...
assertApprox(
  resultWithInsul.dP,
  resultNoInsul.dP,
  1e-2, // 1% tolérance pour itération T_moy
...
console.log(
  `  ℹ️  Sans isolation: Q_loss = ${resultNoInsul.Q_loss.toFixed(1)}W, T_out = ${resultNoInsul.T_out.toFixed(2)}°C`
);
console.log(
  `  ℹ️  Avec isolation: Q_loss = ${resultWithInsul.Q_loss.toFixed(1)}W, T_out = ${resultWithInsul.T_out.toFixed(2)}°C`
);
console.log(
  `  ℹ️  Réduction perte: ${((1 - resultWithInsul.Q_loss / resultNoInsul.Q_loss) * 100).toFixed(1)}%`
);
...
const result1m = pipeSegment.calculatePipeSegment(
  geom1m,
  validFluid,
  validAmbient,
  validInsulation
...
const result10m = pipeSegment.calculatePipeSegment(
  geom10m,
  validFluid,
  validAmbient,
  validInsulation
...
const result100m = pipeSegment.calculatePipeSegment(
  geom100m,
  validFluid,
  validAmbient,
  validInsulation
...
assert(
  result100m.Q_loss > result10m.Q_loss && result10m.Q_loss > result1m.Q_loss,
  'Q_loss devrait augmenter avec la longueur'
);
assert(
  result100m.dP > result10m.dP && result10m.dP > result1m.dP,
  'dP devrait augmenter avec la longueur'
);
assert(
  result1m.T_out > result10m.T_out && result10m.T_out > result100m.T_out,
...
console.log(
  `  ℹ️  1m:   T_out = ${result1m.T_out.toFixed(2)}°C, Q_loss = ${result1m.Q_loss.toFixed(1)}W`
);
console.log(
  `  ℹ️  10m:  T_out = ${result10m.T_out.toFixed(2)}°C, Q_loss = ${result10m.Q_loss.toFixed(1)}W`
);
console.log(
  `  ℹ️  100m: T_out = ${result100m.T_out.toFixed(2)}°C, Q_loss = ${result100m.Q_loss.toFixed(1)}W`
);
...
const resultLow = pipeSegment.calculatePipeSegment(
  validGeometry,
  fluidLow,
  validAmbient,
  validInsulation
...
const resultMed = pipeSegment.calculatePipeSegment(
  validGeometry,
  fluidMed,
  validAmbient,
  validInsulation
...
const resultHigh = pipeSegment.calculatePipeSegment(
  validGeometry,
  fluidHigh,
  validAmbient,
  validInsulation
...
assert(
  resultLow.T_out < resultMed.T_out && resultMed.T_out < resultHigh.T_out,
...
assert(
  resultHigh.dP > resultMed.dP && resultMed.dP > resultLow.dP,
  'dP devrait augmenter avec le débit'
);

// Plus le débit augmente, plus Re augmente
assert(
  resultHigh.Re > resultMed.Re && resultMed.Re > resultLow.Re,
  'Re devrait augmenter avec le débit'
);

console.log(
  `  ℹ️  0.5 kg/s: T_out = ${resultLow.T_out.toFixed(2)}°C, Re = ${resultLow.Re.toFixed(0)}`
);
console.log(
  `  ℹ️  2.0 kg/s: T_out = ${resultMed.T_out.toFixed(2)}°C, Re = ${resultMed.Re.toFixed(0)}`
);
console.log(
  `  ℹ️  5.0 kg/s: T_out = ${resultHigh.T_out.toFixed(2)}°C, Re = ${resultHigh.Re.toFixed(0)}`
);
...
const resultHot = pipeSegment.calculatePipeSegment(
  validGeometry,
  fluidHot,
  validAmbient,
  validInsulation
...
const resultCold = pipeSegment.calculatePipeSegment(
  validGeometry,
  validFluid,
  ambientCold,
  validInsulation
...
const resultNoWind = pipeSegment.calculatePipeSegment(
  validGeometry,
  validFluid,
  ambientNoWind,
  validInsulation
...
assert(
  resultNoWind.h_ext < result10m.h_ext,
  'Pas de vent devrait réduire h_ext (convection naturelle)'
);

// Test 6.4: Vent fort
const ambientStrongWind = { ...validAmbient, V_wind: 20 };
const resultStrongWind = pipeSegment.calculatePipeSegment(
  validGeometry,
  validFluid,
  ambientStrongWind,
  validInsulation
